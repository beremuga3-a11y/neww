...
ANIMAL_CONFIG: List[Tuple[str, int, str, str, str, int, str]] = [
    ...
    ("trrr",  10_000_000, "🦊", "Лунный «Тканевый лис»",    "ultra",
        60_000_000_000_000,
        "Может менять форму своего тела, «растягивая» или «сжимая» собственные нити, тем самым прячься в тканевых лабиринтах."),
    # ------------------- НОВЫЕ ДОРОГИЕ ПИТОМЦЫ -------------------
    ("quantum_hydra",      15_000_000, "🐉", "Квантовая Гидра",           "ultra", 100_000_000_000_000,
        "Существует одновременно в нескольких измерениях, каждая голова генерирует отдельный поток дохода."),
    ("stellar_kraken",     20_000_000, "🐙", "Звёздный Кракен",          "ultra", 150_000_000_000_000,
        "Щупальца протягиваются через галактики, собирая космическую пыль и превращая её в чистое золото."),
    ("void_phoenix",       25_000_000, "🦜", "Феникс Пустоты",           "ultra", 200_000_000_000_000,
        "Возрождается из антиматерии, каждое перерождение увеличивает доход в геометрической прогрессии."),
    ("cosmic_leviathan",   30_000_000, "🐋", "Космический Левиафан",     "ultra", 300_000_000_000_000,
        "Плывёт по потокам тёмной энергии, его песни резонируют с квазарами, генерируя бесконечный доход."),
    ("infinity_dragon",    40_000_000, "🐉", "Дракон Бесконечности",     "ultra", 500_000_000_000_000,
        "Дышит чистой энергией создания, его чешуя – осколки первозданных звёзд."),
    ("omega_unicorn",      50_000_000, "🦄", "Омега Единорог",           "ultra", 750_000_000_000_000,
        "Последний из своего вида, его рог пронзает саму ткань реальности, извлекая богатства из параллельных миров."),
    ("genesis_spider",     75_000_000, "🕷️", "Паук Генезиса",            "ultra", 1_000_000_000_000_000,
        "Ткёт паутину из нитей судьбы, каждая нить – это поток бесконечного богатства."),
    ("eternal_phoenix",    100_000_000, "🦜", "Вечный Феникс",           "ultra", 2_000_000_000_000_000,
        "Не знает смерти, существует вне времени, его перья – кристаллизованная вечность."),
    ("primordial_dragon",  150_000_000, "🐉", "Первозданный Дракон",     "ultra", 5_000_000_000_000_000,
        "Первый дракон, от которого произошли все остальные, его дыхание создаёт новые вселенные."),
    ("absolute_being",     200_000_000, "⚛️", "Абсолютное Существо",     "ultra", 10_000_000_000_000_000,
        "Воплощение самой концепции богатства, его присутствие искажает экономические законы вселенной."),
    # ------------------- МИФИЧЕСКИЕ ГИГАНТЫ -------------------
    ("titan_of_wealth",    300_000_000, "💎", "Титан Богатства",         "mythic", 20_000_000_000_000_000,
        "Древний титан, чьё тело состоит из драгоценных камней размером с планеты."),
    ("god_of_fortune",     500_000_000, "🔱", "Бог Удачи",               "mythic", 50_000_000_000_000_000,
        "Божество, управляющее всеми потоками богатства во вселенной."),
    ("universe_serpent",   750_000_000, "🐍", "Змей Вселенной",          "mythic", 100_000_000_000_000_000,
        "Опоясывает всю вселенную, его движения создают экономические циклы галактик."),
    ("creator_phoenix",    1_000_000_000, "🔥", "Феникс-Создатель",      "mythic", 500_000_000_000_000_000,
        "Создал первые звёзды своим пламенем, каждое перо стоит больше, чем целая галактика."),
    ("omnipotent_spider",  2_000_000_000, "🕸️", "Всемогущий Паук",       "mythic", 1_000_000_000_000_000_000,
        "Его паутина связывает все реальности, контролируя потоки богатства между измерениями."),
]
    ...
    for (uid,) in cur.fetchall():
        ...
        for field, _, _, _, _, _, _ in ANIMAL_CONFIG:
            ...
            if time.time() - last_fed > HUNGER_TIME:
                # Сохраняем информацию о потерянных питомцах
                _execute(
                    """
                    INSERT INTO lost_pets (user_id, pet_field, quantity, lost_time)
                    VALUES (?, ?, ?, ?)
                    """,
                    (uid, field, cnt, int(time.time())),
                )
...
def check_and_reset_season() -> None:
    ...
    if now - row["season_start"] >= SEASON_LENGTH:
        ...
        _execute("DELETE FROM lost_pets")
        new_start = now
...
# ----------------------------------------------------------------------
async def restore_pets(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Восстанавливает всем игрокам питомцев, потерянных за последние 48 часов."""
    if not is_admin(query.from_user.id):
        await edit_section(query, caption="❌ Доступ запрещён.", image_key="admin")
        return
    
    # Получаем питомцев, потерянных за последние 48 часов
    time_limit = int(time.time()) - (48 * 3600)
    cur.execute(
        """
        SELECT user_id, pet_field, SUM(quantity) as total_qty
        FROM lost_pets
        WHERE lost_time > ?
        GROUP BY user_id, pet_field
        """,
        (time_limit,)
    )
    ...
    if not lost_pets:
        await edit_section(
            query,
            caption="❌ Нет питомцев для восстановления (потерянных за последние 48 часов).",
            image_key="admin",
            reply_markup=InlineKeyboardMarkup(
    ...
    for row in lost_pets:
        user_id = row["user_id"]
        pet_field = row["pet_field"]
        quantity = row["total_qty"]
        
        # Получаем текущее количество питомцев у пользователя
        user = get_user(user_id)
        current_qty = user[pet_field]
        new_qty = current_qty + quantity
        
        # Восстанавливаем питомцев
        update_user(user_id, **{pet_field: new_qty})
        set_pet_last_fed(user_id, pet_field, int(time.time()))
        
        restored_count += quantity
        users_affected.add(user_id)
        
        # Записываем в лог
        log_user_action(user_id, f"Восстановлено {quantity} {pet_field} админом")
        
        # Уведомляем пользователя
        try:
            await context.bot.send_message(
                user_id,
                f"✅ Администратор восстановил ваших питомцев:\n"
                f"🔄 {pet_field}: +{quantity} шт.\n"
                f"Не забудьте их покормить!"
            )
        except Exception:
            pass
    
    # Удаляем записи о восстановленных питомцах
    _execute(
        "DELETE FROM lost_pets WHERE lost_time > ?",
        (time_limit,)
    )
    ...
    await edit_section(
        ...
        caption=(
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🔄 Восстановлено питомцев: {restored_count}\n"
            f"👥 Затронуто игроков: {len(users_affected)}\n"
            f"⏱️ Период: последние 48 часов"
        ),
        image_key="admin",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Назад", callback_data="admin")]]
...
async def admin_actions(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "admin_restore_pets":
        await restore_pets(query, context)
        return
