...
async def admin_bot_stats(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    cur.execute("SELECT COUNT(*) as active FROM users WHERE last_reset > ?", (week_ago,))
    active_users = cur.fetchone()["active"]
    
    # Пользователи за последние 24 часа
    day_ago = int(time.time()) - (24 * 3600)
    cur.execute("SELECT COUNT(*) as daily FROM users WHERE last_reset > ?", (day_ago,))
    daily_users = cur.fetchone()["daily"]
    
    # Общее количество монет в системе
    cur.execute("SELECT SUM(coins) as total_coins FROM users")
    total_coins_result = cur.fetchone()
    total_coins = total_coins_result["total_coins"] or 0
    ...
    # Количество фермеров
    cur.execute("SELECT COUNT(*) as farmers FROM farmers")
    ...
    stats_text = (
        ...
        f"🟡 Активных (24 часа): {format_num(daily_users)}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💰 Общее количество монет: {format_num(total_coins)}\n"
        f"👑 Топ пользователь: {top_user_info}\n"
...
async def show_admin_logs(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Показывает улучшенный журнал действий с пагинацией."""
    if not is_admin(query.from_user.id):
        await edit_section(query, caption="❌ Доступ запрещён.", image_key="admin")
        return
    
    page = context.user_data.get("logs_page", 0)
    limit = 15
    offset = page * limit
    
    # Получаем логи с именами пользователей
    cur.execute("""
        SELECT al.user_id, al.action, al.ts, u.username 
        FROM admin_logs al 
    ...
    # Подсчитываем общее количество логов
    ...
    if not rows:
        ...
        for row in rows:
            ...
            # Ограничиваем длину действия
            action = row["action"][:50] + "..." if len(row["action"]) > 50 else row["action"]
            txt += f"[{t}] {username}: {action}\n"
    
    # Кнопки навигации
    nav_btns = []
    if page > 0:
        nav_btns.append(InlineKeyboardButton("⏪ Пред.", callback_data="logs_prev"))
    if page < total_pages - 1:
        nav_btns.append(InlineKeyboardButton("След. ⏩", callback_data="logs_next"))
    nav_btns.append(InlineKeyboardButton("🔄 Обновить", callback_data="admin_view_logs"))
    nav_btns.append(InlineKeyboardButton("⬅️ Назад", callback_data="admin"))
    
    kb = InlineKeyboardMarkup([nav_btns])
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ------------------- Навигация по логам -------------------
    if data == "logs_prev":
        context.user_data["logs_page"] = max(0, context.user_data.get("logs_page", 0) - 1)
        await show_admin_logs(query, context)
        return
    if data == "logs_next":
        context.user_data["logs_page"] = context.user_data.get("logs_page", 0) + 1
        await show_admin_logs(query, context)
        return
    # ------------------- Админ‑панель -------------------
    if data == "admin":
