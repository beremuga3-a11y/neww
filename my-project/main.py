...
def get_bot_stats() -> Dict[str, int]:
    ...
    cur.execute(
        "SELECT COUNT(DISTINCT user_id) as active FROM admin_logs WHERE ts > ?",
        (day_ago,)
    )
    ...
    for field in animal_fields:
        cur.execute(f"SELECT SUM({field}) as sum FROM users")
        total_animals += cur.fetchone()["sum"] or 0
    
    # Количество действий за последние 24 часа
    cur.execute(
        "SELECT COUNT(*) as actions FROM admin_logs WHERE ts > ?",
        (day_ago,)
    )
    actions_24h = cur.fetchone()["actions"]
...
async def feed_animal_step(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user["feed"] == 0:
        await edit_section(
            query,
            caption="❌ У вас нет корма! Купите корм в магазине.",
            image_key="farm",
...
async def feed_animal(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    log_user_action(uid, f"Кормил {animal}")
    set_pet_last_fed(uid, animal, int(time.time()))
    
    await edit_section(
        query,
        caption=f"🍎 {animal.capitalize()} покормлен! +40% дохода на 1 час.",
        image_key="farm",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Назад", callback_data="farm")]]
        ),
...
async def admin_actions(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "admin_bot_stats":
        ...
        txt = (
            ...
            f"🟢 Активных за 24ч: {format_num(stats['active_users'])}\n"
            f"💰 Всего монет в игре: {format_num(stats['total_coins'])}\n"
            f"🐾 Всего животных: {format_num(stats['total_animals'])}\n"
            f"⚡ Действий за 24ч: {format_num(stats['actions_24h'])}\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    if data == "admin_view_logs":
        cur.execute(
            "SELECT user_id, action, ts FROM admin_logs ORDER BY ts DESC LIMIT 30"
        )
        rows = cur.fetchall()
        if not rows:
            txt = "📜 Журнал действий пуст."
        else:
            txt = "📜 Последние 30 действий игроков:\n━━━━━━━━━━━━━━━━━━━━\n"
            for row in rows:
                t = time.strftime("%d.%m %H:%M:%S", time.localtime(row["ts"]))
                # Получаем имя пользователя
                cur.execute("SELECT username FROM users WHERE user_id = ?", (row["user_id"],))
                user_data = cur.fetchone()
                username = user_data["username"] if user_data and user_data["username"] else f"ID{row['user_id']}"
        ...
        await edit_section(
            query,
            caption=txt[:4000],  # Telegram message limit
            image_key="logs",
            reply_markup=InlineKeyboardMarkup([
