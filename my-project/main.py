...
async def admin_bot_stats(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for field, *_ in ANIMAL_CONFIG:
        ...
        total_animals += cur.fetchone()["total"] or 0
...
━━━━━━━━━━━━━━━━━━━━
👥 Общее количество пользователей: {format_num(total_users)}
🟢 Активных за 7 дней: {format_num(active_users)}
🟡 Активных за 24 часа: {format_num(daily_users)}
━━━━━━━━━━━━━━━━━━━━
💰 Общее количество монет: {format_num(total_coins)}
🐾 Общее количество животных: {format_num(total_animals)}
━━━━━━━━━━━━━━━━━━━━
🏆 Топ-5 по монетам:
"""
    ...
    for i, user in enumerate(top_users, 1):
        ...
        stats_text += f"{i}. {name}: {format_num(user['coins'])}🪙\n"
    
    stats_text += f"""━━━━━━━━━━━━━━━━━━━━
⏳ Сезон №{season_number}
🕐 До конца сезона: {h}ч {m}м
━━━━━━━━━━━━━━━━━━━━"""
...
async def admin_view_logs(query, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Показывает улучшенный журнал действий."""
    if not is_admin(query.from_user.id):
        await edit_section(query, caption="❌ Доступ запрещён.", image_key="admin")
        return
    ...
    if not rows:
        ...
        for row in rows:
            ...
            if "Купил" in action:
                action_type = "🛒 Покупки"
            elif "Продал" in action:
                action_type = "📉 Продажи"
            elif "Кормил" in action:
                action_type = "🍎 Кормление"
            elif "Получено" in action:
                action_type = "💰 Доход"
            elif "Выиграл" in action or "Проиграл" in action:
                action_type = "🎰 Казино"
            elif "Кликнул" in action:
                action_type = "👆 Клики"
            elif "Активировал промокод" in action:
                action_type = "🎟️ Промокоды"
            else:
                action_type = "📝 Прочее"
        ...
        txt += "📊 Статистика по типам:\n"
        for action_type, count in sorted(action_counts.items(), key=lambda x: x[1], reverse=True):
            txt += f"{action_type}: {count}\n"
        
        txt += "\n━━━━━━━━━━━━━━━━━━━━\n"
        txt += "📋 Последние 15 действий:\n"
        
        # Показываем последние 15 действий с именами пользователей
        for i, row in enumerate(rows[:15], 1):
            ...
            txt += f"{i:2d}. [{t}] {username}: {action}\n"
    
    # Добавляем кнопки навигации
    btns = [
        InlineKeyboardButton("🔄 Обновить", callback_data="admin_view_logs"),
        InlineKeyboardButton("📊 Статистика", callback_data="admin_bot_stats"),
        InlineKeyboardButton("⬅️ Назад", callback_data="admin")
    ]
